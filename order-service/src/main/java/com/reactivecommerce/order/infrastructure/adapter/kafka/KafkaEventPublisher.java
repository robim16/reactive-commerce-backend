package com.reactivecommerce.order.infrastructure.adapter.kafka;
 ...
 *
 * Publica los eventos de dominio del Order Service:
 *   order.created, order.completed, order.payment_failed, order.refunded
 *
 * El producer usa ACKS=all + idempotencia para garantizar exactly-once
 * en conjunto con el Outbox Pattern del repositorio.
 */
@Slf4j
@Component
...
public class KafkaEventPublisher implements DomainEventPublisher {
    ...
    public Mono<Void> publish(String topic, String key, Object event) {
        return Mono.fromCallable(() -> objectMapper.writeValueAsString(event))
            ...
            .as(records -> kafkaSender.send(records))
            .next()
